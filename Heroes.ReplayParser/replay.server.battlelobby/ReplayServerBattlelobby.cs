			...
			using (var stream = new MemoryStream(buffer)) {
				...
				if (replay.Players != null) {
					foreach (var player in replay.Players.Where(i => i != null)) {
						...
						while (!reader.EndOfStream) {
							...
							for (var i = 0; i < playerNameBytes.Length + 1; i++)
								if ((i == playerNameBytes.Length && reader.ReadByte() != 35 /* '#' Character */) ||
								    (i < playerNameBytes.Length && reader.ReadByte() != playerNameBytes[i])) {
									isFound = false;
									break;
								}
					...
					if (replay.Players.Any(i => i != null && i.BattleTag == 0))
						Debug.WriteLine("Bot found or player missing");
				}
			}
		}
	}
