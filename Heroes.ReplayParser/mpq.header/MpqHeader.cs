	...
	public static class MpqHeader {
		...
		public static void ParseHeader(Replay replay, string filename) {
			using (var reader = new BinaryReader(new FileStream(filename, FileMode.Open))) {
				...
				try {
					if (headerStructure.dictionary != null) {
						replay.ReplayVersion = string.Format("{0}.{1}.{2}.{3}", headerStructure.dictionary[1].dictionary[0].vInt.Value,
							headerStructure.dictionary[1].dictionary[1].vInt.Value, headerStructure.dictionary[1].dictionary[2].vInt.Value,
							headerStructure.dictionary[1].dictionary[3].vInt.Value);
						replay.ReplayBuild = (int) headerStructure.dictionary[1].dictionary[4].vInt.Value;
					}
					else {
						replay.ReplayVersion = "0.0.0.0";
						replay.ReplayBuild = 0;
					}
				} catch (Exception e) {
					replay.ReplayVersion = "0.0.0.0";
					replay.ReplayBuild = 0;
