	...
	public static class ReplayDetails {
		...
		public static void Parse(Replay replay, byte[] buffer) {
			...
			using (var reader = new BinaryReader(stream)) {
				...
				if (replayDetailsStructure.dictionary != null)
				...
				if (replay.Players != null && replay.Players.Length != 10)
					// Try Me Mode, or something strange
					return;

				
				var playerIndexes = replay.TrackerEvents.Where(i => i.TrackerEventType == ReplayTrackerEvents.TrackerEventType.PlayerSetupEvent &&  i.Data.dictionary != null && i.Data.dictionary[2].optionalData != null).Select(i => i.Data.dictionary[2].optionalData.vInt.Value).OrderBy(i => i).ToArray();
				...
				// [5] - Utc Timestamp
				if (replayDetailsStructure.dictionary != null) {
					var vInt = replayDetailsStructure.dictionary[5].vInt;
					if (vInt != null)
						replay.Timestamp = DateTime.FromFileTimeUtc(vInt.Value);
				}
